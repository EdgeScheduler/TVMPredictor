...
class op_graph:
    ...
    def traverse_and_calculate_per_op(self, ir_params, x, input_name, device, target):
        """
        ...
        while len(available_op_queue) > 0:
            ...
            op_list = [temp_op]
            temp_op_list = []
            if temp_op.concentrate is not False:
...
def profile_resource_usage(ir_params, x, input_name, device=tvm.cuda(0), target="cuda"):
    return computation_graph.traverse_and_calculate_per_op(ir_params, x, input_name, device, target)
