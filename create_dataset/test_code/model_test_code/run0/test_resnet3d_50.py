...
# 定义参数
function_dict = {"func":calculate_model, "name": "resnet3d-50"}
...
for a in uniform_sampling(16,256,0.0625):
    for b in uniform_sampling(16,256,0.0625):
        ...
        generate_datasets_with_one_dimensionality_changing(device_parame_array=[Device.device_params_GPU0,],count=count,shape_dimensionality=shapes_dimensionality,range_min=range_min,range_max=range_max,function_dict = function_dict,min_shapes=common_args.min_shapes,max_shapes=common_args.max_shapes,sampling=common_args.sampling,force_shape_relation=force_shape_relation,dtype=common_args.dtype,cycle_times=common_args.cycle_times,min_repeat_ms=common_args.min_repeat_ms,opt_level=common_args.opt_level,fold_path=common_args.fold_path,device_name=common_args.device_name,show_print=common_args.show_print,isModule=common_args.isModel,dataset_config_name="dataset_dell04_GPU0.json")
        count+=1