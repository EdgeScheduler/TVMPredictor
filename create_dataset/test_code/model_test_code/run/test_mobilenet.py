...
# 定义参数
function_dict = {"func":calculate_model, "name": "mobilenet"}
...
for a in uniform_sampling(16,256,0.0625):
    for b in uniform_sampling(16,256,0.0625):
        ...
        for device_parame in common_args.device_parame_array:
            generate_dataset_with_one_dimensionality_changing(function_dict=function_dict,shapes=shapes,min_shapes=common_args.min_shapes,max_shapes=100,sampling=common_args.sampling,force_shape_relation=force_shape_relation,device_parames=device_parame,dtype=common_args.dtype,cycle_times=common_args.cycle_times,min_repeat_ms=common_args.min_repeat_ms,opt_level=common_args.opt_level,prefix_path=common_args.prefix_path,fold_path=common_args.fold_path,device_name=common_args.device_name,dataset_config_name=common_args.config_name,show_print=common_args.show_print,isModule=common_args.isModel)