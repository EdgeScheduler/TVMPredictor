...
from create_dataset.datasets.common import get_database_json,read_data,mycolor,myplot,data_div
...
model_list = ["inception_v3","squeezenet_v1_1","mobilenet","resnet-50","resnet3d-50"]
...
for i in range(len(model_list)):
    ...
    for j in range(len(device_list)):
        if j==0:
            ...
            if not datas_1:
                print("data_1 is None")
                break
            
            img = plt.subplot(len(device_list)+1,len(model_list)+1, i+1+j*(len(model_list)+1))
            myplot(data=datas_1[0],color=mycolor(i+1),label_type="shape")
            img.set_title("GPU-"+device_list[j]+": "+model_list[i])

            img_sum = plt.subplot(len(device_list)+1,len(model_list)+1, (j+1)*(len(model_list)+1))
